    ...
    dbCon().query(sql, async (err) => {
        if (err) {
            ...
            for (var i in json_data) {
                userControlArr.push([json_data[i]['salesmanId'],
                json_data[i]['salesmanName'],
                ...
                var sql2 = "INSERT INTO " + tableName + " (salesmanId,\
                salesmanName,\
                addressLine1,\
                addressLine2,\
                addressLine3,\
...
// get all getsalesmanMaster
const getsalesmanMaster = (req, res) => {
    let databaseName = req.body.databaseName;
    let sql = "USE " + databaseName;
    dbCon().query(sql, async (err) => {
        if (err) {
