    ...
    dbCon().query(sql, async (err) => {
        if (err) {
            ...
            for (var i in json_data) {
                userControlArr.push([json_data[i]['userId'], json_data[i]['brokerId'],
                json_data[i]['brokerName'],
                ...
                var sql2 = "INSERT INTO " + tableName + " (brokerId,\
                brokerName,\
                addressLine1,\
                addressLine2,\
                addressLine3,\
...
// get all getbrokerMaster
const getbrokerMaster = (req, res) => {
    let databaseName = req.body.databaseName;
    let sql = "USE " + databaseName;
    dbCon().query(sql, async (err) => {
        if (err) {
